...
Once plans are written down, nothing stops you from running several agents at once.
...
> [!tip]
> Worktrees pair very well with the plan-first workflow: write N plans with one strong agent, then fan out N cheaper agents, each implementing one plan in its own worktree.
