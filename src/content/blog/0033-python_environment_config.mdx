...
There are some methodologies that you can follow when working with **GIT**, my suggestion is that you use **gitflow**: <FancyInternalLink slug="using-gitflow"/>
...
My personal favorite is **cmder**: <FancyInternalLink slug="cmder-terminal-windows"/>
...
The more secure one is by setting up a **SSH** key: <FancyInternalLink slug="git-with-ssh"/>
...
As an example you can use `black` with `sublack` for **Sublime Text**: <FancyInternalLink slug="black-code-formatting"/>
...
You can read more about it here: <FancyInternalLink slug="poetry-python-package-manager"/>
...
There are multiple way of using them. What I suggest is that you use `pre-commit`: <FancyInternalLink slug="pre-commit"/>
