...
If you haven't read that post, see <FancyInternalLink slug="running-dbt-with-aws-ecs-fargate"/>.
...
More info about it in <FancyInternalLink slug="pre-commit"/>.
...
That could be `dbt build --select xx --exclude yy` or using a Python script as described in <FancyInternalLink slug="running-dbt-with-aws-ecs-fargate"/>.
...
In production, I'm using Athena (see <FancyInternalLink slug="running-dbt-with-aws-ecs-fargate"/>) and there the default catalog is called `awsdatacatalog`.
...
So instead of adding the `optimize` there like:
