...
The calls to `Chat` are very similar to the ones for `Transcriptions`.
The differences are that we now have to handle the number of tokens and that we want the output to be structured as a pydantic class (more details on this can be found in <FancyLink linkText="Process Calls with Open AI" url="https://villoro.com/blog/process-calls-open-ai/" dark="true"/>).
