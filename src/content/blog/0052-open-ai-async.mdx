...
We use a Rate Limiter to prevent exceeding API usage limits, ensuring compliance and avoiding throttling or service disruptions. This helps us get more valid requests per minute.

### 3.1. How does it work?

* **Managing Requests and Tokens:** Before making an API call, the limiter checks if there are sufficient remaining requests or tokens. If limits are reached, it waits until the limits reset.

* **Handling API Responses:** After each API call, the limiter can update its state using information from response headers, keeping its internal counters accurate. More on that on the next section
