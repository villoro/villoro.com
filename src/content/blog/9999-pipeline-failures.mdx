...
Sometimes you don’t want the failure of a single task to stop an entire pipeline. For example, you may be downloading multiple tables from a CRM, and if one of them fails, you'd still want to download the rest rather than stopping the whole process. Later, you can handle and report any failures encountered.

In this section, we'll show how to implement this behavior in Prefect for both async and sync flows.
...
def main():
    ...
    futures = [process_item.submit(i, return_state=True) for i in items]
    failures = []

    for state, item in zip(futures, items):
