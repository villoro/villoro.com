...
### 3.1. Strategies to Handle Failures
...
The idea is to be able to know which models succeed and which failed.

This can be done by:

1. Get a unique id for the execution
1. Exporting DBT run results.
2. Reading run results.

### 3.2.1. Getting a unique execution id

It is really important to pass some id to DBT so that we can know match the run results with the actual run.

With prefect it can be done with:
...
### 3.2.2. Exporting DBT run results
...
The important part is to pass the `flow_run_id` to DBT so that it can be added as a column.
This will allow us to filter later the run results we want.

### 3.2.3. Reading the results
...
def are_results_good(df, dbt_expectations=None, models=None, tests=None):
    logger = get_run_logger()
    ...
    if dbt_expectations is None:
        if models is None and tests is None:
            logger.info("There are no dependencies")
            return True
...
async def post_dbt(flow_run_id, env: Literal["snd", "pro"] = "pro"):
    ...
    jobs = []
    for func, expectations in POST_DBT_JOBS.items():
...
Failures in upload jobs need no special treatment since there are no jobs that dependen on them.
