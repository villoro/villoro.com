...
The first thing you will need to do is to create a `Prefect Block` (more info at <FancyLink linkText="Prefect Essentials | Using prefect blocks" url="https://villoro.com/blog/prefect-essentials-setup-and-migration/#using-prefect-blocks" dark="true"/>) to store the `GitHub` credentials if the repo is not public.
