...
The first thing you will need to do is to create a `Prefect Block` (more info at <FancyInternalLink slug="prefect-essentials-setup-and-migration" anchor="3-using-prefect-blocks"/>) to store the `GitHub` credentials if the repo is not public.
