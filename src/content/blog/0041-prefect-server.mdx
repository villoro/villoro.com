...
Now that we have a working server we want to connect everything that runs `flows` to the `server`.
This way we can track everything.

To connect to the server we only the following environment variable:
...
We need to set an argument for the `API URL` and then store it as an env variable with:
...
2. Having a central `server` that can trigger runs to multiple/any enviroment

With a dedicated server you will have better isolation but it would be more work to maintain and will have greater costs.
In my case I wanted simplicity a lower costs so I went with a central server.
...
## 5. Useful code snipets

Here you have some snipets that can help you getting started.
...
In my case it's very useful for tagging the `enviroment` where the `flow` is running (such as `env:pro` or `env:pre`).
...
### 5.3. Defining a flow per each possible lambda

With the `base.py` code for running lambdas you can define a `flow` per each `lambda` you have.
