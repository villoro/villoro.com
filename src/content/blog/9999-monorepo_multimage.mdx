...
* **Per-lib workspaces** *(preferred here)*: each internal package (e.g., `nt_common`, `nt_rds`) is independently versioned and locked. This allows for better modularity, selective releases, and independent dependency resolution.
...
* Accepts `--build-arg PACKAGE_NAME=nt_rds`
...
```yaml
run: docker build --build-arg PACKAGE_NAME=nt_rds ...
```
