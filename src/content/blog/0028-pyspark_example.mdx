...
Be sure to read <FancyInternalLink slug="pyspark-intro"/> before proceding with this post.
...
Some of those could be `sdf.show`, `sdf.toPandas` or writing a file.
...
The problem is that we are reading from a `csv` which is a really inefficient file format.
...
To do so we need to create a `udf` and then apply it:
