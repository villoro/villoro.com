...
Below is a minimal version tagging script implemented with **Click**.
...
<Notice type="success">
  The hook reads the current version, checks if a tag with that name already exists, and creates it if not.
</Notice>

### 1.4 Consuming the Hook from Any Repo

Add a workflow that runs on `main` pushes and tags automatically:
...
Expressions like `${{ hashFiles() }}` don’t work inside the matrix definition. Compute them at runtime instead:

```yaml
key: uv-${{ runner.os }}-${{ hashFiles(format('src/{0}/uv.lock', matrix.name)) }}
```

### 2.3. Use Filters to Trigger Only What’s Needed
...
## 3. Gate Jobs — One Check to Rule Them All
...
Your CI should scale with your codebase — not slow it down.
