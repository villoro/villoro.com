...
The key here is to use structured outputs that were anounced some weeks ago (<FancyLink linkText="Introducing Structured Outputs in the API" url="https://openai.com/index/introducing-structured-outputs-in-the-api/"/>).
Also take a look at the documentation at <FancyLink linkText="API Reference | response_format" url="https://platform.openai.com/docs/api-reference/chat/create#chat-create-response_format"/> and <FancyLink linkText="Structured Outputs" url="https://platform.openai.com/docs/guides/structured-outputs/introduction"/>.

<Notice type="warning">
  Notice that there is a way to provide a Pydantic class directly in the API call and that it will handle the casting.
  However this is in beta and I prefer not using it yet.
</Notice>

### 5.1. Creating the prompt with a pydantic class

In order to avoid code duplication, the first thing we will do is to create a <FancyLink linkText="Pydantic" url="https://docs.pydantic.dev/"/> class that the output should follow:
...
We can use the pydantic class for creating the prompt with `CallAnalysis.model_json_schema()`.
...
The important part here is handling the `response_format` properly.
With `get_json_schema_from_pydantic` it can be created based on the `pydantic` object.

Then you can get the output with `CLIENT.chat.completions.create`.

Finally, in order to store relevant metadata about the API call I extend the pydantic class and add some relevant fields.
...
def extract_info(transcription)
    ...
    response_format = get_json_schema_from_pydantic(CallAnalysis)
