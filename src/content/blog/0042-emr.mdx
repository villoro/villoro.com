...
No more tangled messes—just straightforward steps and clear traceability.
...
You have seen how to add external packages and how to install your own code as a package.
However you still need an `entrypoint` for running `EMR` jobs.
Given that any python file you have in your code could be an `entrypoint` you would be force to **upload all files to S3**.

Luckily there is a better way for handling this.
The idea is to have a generic `entrypoint.py` that runs code from other packages.
...
With that, we can pass an argument to the generic entrypoint in a way that ends up importing the file we need and running the `main` function inside.
...
However we will be doing the import dynamically so that it can be done based on the path as a string.
This is done with:
...
One way to improve the previous code is to define the parameters as a `pydantic` class.
This way you don't need to handle the `castings` and can use the powerful validators from pydantic.
...
For example, let's imagine you are extracting multiple tables from somewhere.
Here you could create a `prefect.task` for each table.
...
* A decorator that catch the `Params` and that creates a `prefect.flow`
