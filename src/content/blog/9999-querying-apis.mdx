...
Don’t just react to 429s — **pace your calls proactively**. A minimal, readable approach is to space requests evenly and honor common headers when present.
...
<Notice type="warning">
  Rate limits are provider‑specific. Prefer header‑driven sleeps (`Retry-After`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`) over hardcoded delays. Check docs, e.g. [GitHub API rate limits](https://docs.github.com/en/rest/overview/resources-in-the-rest-api#rate-limiting).
</Notice>

For advanced concurrency + rate limiting, see <FancyLink linkText="Concurrent rate‑limited requests" url="/blog/xxxx-concurrent-rate-limit" dark="true"/>.
...
<Notice type="warning">
  You will need to handle deduplication later.
</Notice>
