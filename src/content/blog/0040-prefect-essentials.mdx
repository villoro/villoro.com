...
There are some times where flow runs appear as `Running` even if they are not.
For example that frequently happens when an EMR job times out.

The first thing is to get the running flow runs:
...
async def get_running_flow_runs(start_time_min=datetime.now() - timedelta(days=1)):
    ...
    data = await client.read_flow_runs(flow_run_filter=flow_run_filter)

    df = pd.DataFrame([x.dict() for x in data])
    return df.sort_values("created")
```

That will give you a pandas dataframe with all running flow runs.

<Notice type="warning">
  You might not want to cancell all flow runs, filter it accordingly.
</Notice>

Then you can cancell the flow runs with:

```python
def cancell_flow_runs(flow_run_ids):
    for x in flow_run_ids:
        await client.set_flow_run_state(str(x), state=states.Cancelled(), force=True)

df = await get_running_flow_runs()
cancell_flow_runs(flow_run_ids=df["id"])
```
