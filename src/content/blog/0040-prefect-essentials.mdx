...
I think that migrating from another orquestrator to `prefect` can be easily done in incremental steps:
...
The idea behind those steps is to be able to test `prefect` and see if it indeeds suits better your needs compared to other solutions.
...
The idea is to use `prefect` only for `observability` purpouses so that you have a centralized place from where you can:
...
For example you could have a the following `flows`:
...
At this point you will need to decide if you want to set up you Prefect server.
...
<Notice type="warning">
  Now is the time to finally switch to `Prefect` and to deprecate any other orquestrator you might have
</Notice>
