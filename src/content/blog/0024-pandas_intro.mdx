...
The 2 main disadvantatges is that it will try to fit all the information in the RAM memory so it has limits on the amount of data it can work with.
...
For example you can export it to `parquet` (a really efficient file format, more info <FancyInternalLink slug="storing-tables-efficiently-pandas"/>).
...
In the future I will write another post with more advanced **pandas** tricks.
