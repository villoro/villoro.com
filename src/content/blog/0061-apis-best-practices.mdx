...
<Notice type="error" className="mt-6">
    Don’t exit from <code>finally</code>: no <code>return</code>, <code>break</code>, or <code>continue</code>. These suppress any exception from <code>try</code>. Also avoid raising new errors here, wrap exports so failures don’t mask the original. If you need to return a value, do it **after** the <code>try/finally</code>.
</Notice>
