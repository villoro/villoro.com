...
This can happen if your network, VPN, firewall, or ISP handles SSH traffic on port `22` badly. GitHub also supports SSH over port `443`, which is usually treated like normal HTTPS traffic.

To force GitHub SSH to use port `443`, create or edit this file:
...
If cloning is still slow, another possible issue is bad IPv6 routing. You can force SSH to use IPv4 by adding `AddressFamily inet`:
