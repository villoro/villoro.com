...
We could be running DBT as a CLI but we wouldn't be able to:

* Integrate with `Prefect`
* Add custom logs for certain events
* Export results as a table

This is why I use <FancyLink linkText="Programmatic invocations" url="https://docs.getdbt.com/reference/programmatic-invocations"/> instead.
It boils down to calling `dbtRunner` such as:
...
<Notice type="info">
  Remember that you need to connect to prefect, more info at <FancyLink linkText="Connect Docker | Setting Up and Deploying Prefect Server" url="https://villoro.com/blog/prefect-server-setup-configuration-deployment#31-connect-docker/" dark="true"/>
</Notice>

### 6.1. Tasks

In order to have good observability I suggest having each DBT command as a `prefect` task.
This is as simple as adding the `task` decorator, like:
...
One really useful DBT feature is the ability to registrer callbacks (<FancyLink linkText="Registering callbacks | Programmatic invocations" url="https://docs.getdbt.com/reference/programmatic-invocations#registering-callbacks"/>).
With them you can easily add custom logs.
This is specially useful in order to send logs from `DBT` to `prefect`.
To do so you basically need to do 2 things:

1. Create a callback for handling the logs
2. Set up the logger. Since it needs to be tied to a prefect `task` it's not straightforward

#### 6.2.1. Create a `callback`

The idea here is to catch information from the `DBT` events and send the log based on the level.
...
<Notice type="info">
  In my case I wanted to have `CommandCompleted` as `info` instead of `debug`, but feel free to ignore that `if`.
</Notice>

In my opinion there are some events that I find redundant or not useful at all.
This is way I restrict the callback to a subset of all the events present.
Those are the ones I decided to log:
...
#### 6.2.2. Set up the logger

You need to declare a **global** `logger` in a function that is called inside a prefect `task` or `flow`.
...
I think that there are two things you might want to export:
...
The idea here is to combinte the following files into one:
...
<Notice type="success" className="mt-6">
  Once the exported you simply need to upload the `docs` file into `S3`.
</Notice>
...
## 8. `run.py` complete scripts

We have seen parts of the `run.py` but I think it's useful to see the complete code.
...
<Notice type="warning" className="mt-6">
  The `exit(0)` and `exit(1)` are necessary to let `ECS` know that outcome of the task.
  Without this it would look like as if the task never fails.
</Notice>
