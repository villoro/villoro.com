...
So the solution is to create a `venv.tar.gz` file that contains all `DBT` code and that is downloaded at execution time.
Those `venv` are versioned so that we can easily use older versions if needed.
...
* <FancyLink linkText="How to Build and Push Docker Images to AWS ECR" url="https://www.freecodecamp.org/news/build-and-push-docker-images-to-aws-ecr/"/>
