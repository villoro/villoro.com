...
<Notice type="error">
  **Disclaimer:** After some years, I'd suggest you go with [github flow](https://guides.github.com/introduction/flow/) whenever possible since it greatly simplifies the workflow.
  And only switch to [gitflow](https://danielkummer.github.io/git-flow-cheatsheet/) if you actually needed to have **2 long lived branches** (`develop` + `main`)
</Notice>
...
If you are using [sourcetree](https://www.sourcetreeapp.com/) you can use the **gitflow** button on the top right corner or simply press `CTRL` + `SHIFT` + `F`.
