...
The simplest way of smoothing a time series is by reducing the granularity.
...
We can apply the Gaussian smoothing using <FancyLink linkText="tsmoothie" url="https://github.com/cerlymarco/tsmoothie/tree/master" dark="true"/>:
...
We can also apply the Lowess smoothing using <FancyLink linkText="tsmoothie" url="https://github.com/cerlymarco/tsmoothie/tree/master" dark="true"/>:
...
There is no standard for calculating the quantiles in SQL. As an example, this is how it can be done with <FancyLink linkText="DuckDB" url="https://duckdb.org/" dark="true"/>.
...
Also you can partition by one or more categories with:
