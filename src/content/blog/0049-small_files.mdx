...
So in a lot of cases the best partition strategy is to not partition at all.
Given that we will be appending new data based on the `updated_at`/`extracted_at` column, we will be able to use `row groups` (see: <FancyLink linkText="Parquet | Concepts" url="https://parquet.apache.org/docs/concepts/" company="parquet"/>) for effective data prunning even if there are no partitions.

With Iceberg we can follow a similar strategy were small tables are not partitioned.
In the end, both tables use `parquet` under the hood, so they can use the same techniches for smart data pruning.
However with Iceberg we have even more options. Iceberg provides functions to partition the data effectively, such as `months(extracted_at)`, which can reduce the number of partitions and improve query performance. See <FancyLink linkText="Iceberg | Partition Transforms" url="https://iceberg.apache.org/spec/#partition-transforms" company="iceberg"/>.
