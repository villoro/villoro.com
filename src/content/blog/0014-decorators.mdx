...
This takes `name` as a parameter. It is even possible to define functions inside functions and a child function would be able to use its parent parameters.
...
For example let's create a function that will create an `html` title by adding `<h1>` and `</h1>` around:
...
This will print `Done in 3.00 seconds` and the output is `<h1>hello Turtle</h1>`
...
The interesting part is that we can use other outputting functions instead of `print`, for example some `logging`:
...
```
<TerminalOutput color="red">
  **ERROR:root:** Error in 'useless_function': division by zero
</TerminalOutput>
<TerminalOutput>
  Done in 2.00 seconds
</TerminalOutput>

This logs `ERROR:root:Error in 'useless_function': division by zero` and print `Done in 2.00 seconds`.

After that post you should now understand python **decorators** and be able to create your own. If you have a really good idea for a decorator write it down in the comments!
