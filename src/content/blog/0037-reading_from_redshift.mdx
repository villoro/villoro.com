...
As an example this can be done with **SQL alchemy**: <FancyInternalLink slug="sql-alchemy-python"/>
...
As usual, the suggestion is to use **parquet** for storing columnar data. More info here: <FancyInternalLink slug="storing-tables-efficiently-pandas"/>
...
That means that **pyarrow** is only retrieving some `row_groups` and therefore less data needs to be read.
