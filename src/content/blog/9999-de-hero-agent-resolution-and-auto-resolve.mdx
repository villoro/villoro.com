...
{/* SCREENSHOT: summary page at / — filters row, stat cards, failure list. File: ../../images/posts/2026/9999-de-hero-summary-page.png */}

The filters are a time window (default: the last 24 hours), job name, source app, and resolution state. Resolution state defaults to *unresolved*, since that is what a hero on call actually cares about.

Above the list sit four stat cards: total, resolved, unresolved, and won't-fix. There is one deliberate choice in how they count.

> [!tip]
> The stat cards count the whole window regardless of the resolved/unresolved filter. If they shrank to match the filter, switching to "resolved" would show "resolved: N of N", which tells you nothing. Keeping them fixed lets the hero read "12 total, 9 resolved, 3 left" while filtering the list down to just those 3.
...
{/* SCREENSHOT: detail page at /failures/{id} — context, suggested-resolution pill next to the resolution form. File: ../../images/posts/2026/9999-de-hero-detail-page.png */}

This is where the suggested resolution from the last post earns its place. The model's suggestion is shown as a pill right next to the control the hero uses to set the real resolution, styled the same as the resolution pill so the two read as a matched pair. The hero can accept the suggestion or override it; either way it is one click.
...
Two subtleties, both of which were real bugs I had to fix.

First, the params themselves. ECS reports them JSON-encoded under `metadata["params"]`, next to run and environment noise like `duration_minutes` and `ecs_version`. Matching on the raw metadata dict never worked, because `duration_minutes` alone guarantees two runs never match exactly. The fix was to decode `metadata["params"]` and match on that, ignoring the noise:
