...
<Notice type="info">
  Use feature toggles to **simulate side effects** in dev — log instead of sending emails, skip destructive operations, mock external APIs.
</Notice>
...
> DIAGRAM IDEA: show a shared warehouse with `prod_salesforce` alongside `alice_salesforce` and `bob_salesforce`, arrows pointing to how dev and prod jobs isolate outputs.
...
> DIAGRAM IDEA: compare two flows side by side — traditional multi-env (dev → test → prod) vs. prod-only with prefixed schemas.
