...
SQL is one of the most relevant languages for databases. So at one point you will need to interact to get, store or modify some data. Fortunately you can do it in python using **SQL Alchemy**.

In this post I will show how to do it while giving some advice based on problems I had.
...
For the `drivername` and `port` below you have an example for some popular SQL databases:
...
Ideally to insert data to SQL is as easy as calling the dataframe function `to_sql`. For example:
...
<Notice type="warning">
  Be careful with using `if_exists="replace"` because it will delete the SQL table and create it another time. This caused me some problems when I was inserting Japanese text in a table because the encoding was lost in the process.
</Notice>
