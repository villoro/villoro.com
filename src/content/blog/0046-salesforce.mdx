...
1. They have changed
2. An external entity that is used in a formula has changed

On the second scenario you don't have an easy way to distinguish if the change is on a filed that is used on the formulas or on other fields.
That means that you will consistenly download more data that what is really needed.

In general this is not a problem but in some cases it can lead to too much data being downloaded.
For example, let's imagine we are querying `orders` and that there is calculated field that uses the `products` entity. The `orders` table is really big but there are no massive changes to entries there. However we have very few `products`, so any change in one of them will lead to a lot of `orders` being download. That can greatly slow down the extractions since a lot of data will be extracted.

<Notice type="warning">
  Be careful with the amount of data you need to extract when doing compound queries.
</Notice>

And it could be even worse. As an example, we have an automated process that updates all products at night. That means that we need to download all `orders` daily. The solution we found here was to disable some external entities (such as `products` in `orders`) and remember to not rely on fields in the `orders` table that derive from `product`, since we don't have a way to guarantee that they are acurate. Other alternatives could be:

* Letting the long process run and assume the cost
* Triggering the pipeline more frequently, which could led to smaller batches
