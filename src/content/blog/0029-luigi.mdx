...
This, for example, make it possible to call tasks directly from python and/or to set task dependencies dynamically.
...
> [!warning]
> All **Luigi** tasks need to end with writing a file. This is how luigi tracks which tasks are completed.
...
It is even possible to adapt the `StandardTask` to do something different by overwriting the `run_std` function.
For example:
...
Then create some auxiliary folders that you will need:
...
# For cron logs (and give write permission)
...
Be careful to set `user` to one that has visibility of the `luigid` file.
