...
We hit a strange issue with Athena: some queries started failing silently, while others returned cryptic errors in the DBT logs. After digging into multiple layers (Athena, dbt, Glue Catalog, Iceberg...), we finally discovered the culprit: **Glue table version limits**.

![DBT athena logs failure](../../images/posts/2025/9999-dbt-athena-failure.jpg)

Until then, I was 100% unaware that:
1. There is a hard limit of **1,000,000 schema versions across the entire Glue catalog**.
2. Exceeding this limit doesn't just block updates — it can **break Athena queries** in ways that aren't obvious.
...
![Number of versions by table](../../images/posts/2025/9999-table-versions.png)
...
def get_tables(database, limit=1000):
    ...
    df = wr.catalog.tables(database=database, limit=1000)
...
def clean_old_glue_table_versions(
    ...
    if len(versions) <= min_versions:
        logger.debug(
            f"{prefix} has {len(versions)} versions < {min_versions=}, nothing to do."
        )
        return 0
...
def clean_one_database(database):
    ...
    for table in tables:
        try:
            ...
            # If the table is deleted, simply skip
...
What surprised us the most was this: **query performance improved dramatically** after cleanup.

- Metadata reads are faster.
- Query planning is lighter.
- Maybe Glue stops doing deep version resolution?
