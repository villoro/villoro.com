---
slug: aws-lambdas-to-control-ec2
title: Using AWS Lambda to control EC2 instances
...
date: 2019-04-23
image: "/images/blog/0015-aws-lambda.jpg"
...
This lambda allows users to start or stop instances through html requests. There is a URL and users can specify in **queryStringParameters** which system they want to act upon and what action they want (`start/stop/status`). For security reasons it is also needed to include a **token** in the headers.

An example of the data used would be:
```python
headers = {
  "token": ******
}
...
<Notice type="warning">
  **Important:** use the appropriate region.
</Notice>
...
If the token is correct it will iterate over the **systems** present in the request params and if it is found in the _INSTANCES_ dictionary and if the **action** is ```start/stop``` it will act. The lambda will return a json with the result of each system, like this:
...
![AWS lambda screenshot](../../images/posts/2019/0015-aws_lambda_screenshot.png)
  ...
  <Notice type="info">
    API Gateway is used as a trigger. Click at the left of the image, where it says add trigger.
  </Notice>
...
And you need to set the time trigger by using AWS CloudWatch.

1. Go to AWS CloudWatch service
2. Go to sidebar ```Events/rules```
    1. Select **schedule**
    2. Select **cron expression** and write expected cron
