...
## 2. Building and Publishing the venv (`Dockerfile.venv` + `upload_all.py`)
...
### 2.5 Publishing the artifact (`upload_all.py`)
...
## 3. The Runtime (image, setup & download)
...
### 3.1 Runtime Image (`Dockerfile.runtime`)
...
### 3.2 Runtime Bootstrap Script (`setup_and_run.sh`)
...
### 3.3 Download Script (`download_all.py`)
...
We keep **testing**, **venv publishing**, and **runtime image pushes** as separate jobs.
This speeds up iteration (you rarely need to rebuild the runtime docker image) and preserves reproducibility (each venv is versioned).

### 4.1. CI: Tests with `uv` (quick and cache-friendly)
...
Triggers when app code, dependency files, or deployment config changes. Produces a **versioned quartet**: `pyproject_<v>.toml`, `uv_<v>.lock`, `entrypoint_<v>.py`, `venv_<v>.tar.gz`. Optionally also tags `latest`.
...
* **Gate CD on CI**: require the test job to pass before venv/image jobs run.
...
* **Use OIDC**: you’re using GitHub→AWS OIDC; no static secrets.
...
<Notice type="success">
  Deploy flow: merge to `main` → CI runs tests → CD builds `venv_<version>.tar.gz` and uploads versioned artifacts → runtime image is rebuilt only when its sources change → ECS tasks select the desired version at launch (`-v <version>`).
</Notice>
