...
In a previous post (<FancyInternalLink slug="transcribe-audios-whisper-extract-structured-data-chatgpt"/>), we explored how to extract insights from audio recordings using OpenAI models.
...
def count_tokens(messages, model=MODEL_TOKENIZER_DEFAULT):
    ...
    # I believe the tokenization does not change between sub models
...
As seen in <FancyInternalLink slug="transcribe-audios-whisper-extract-structured-data-chatgpt"/> we were using the <FancyLink linkText="OpenAI python" url="https://github.com/openai/openai-python" dark="true"/> package.
...
The `call_openai_transcription` function manages the preparation of input data for the API call and formats the output as a Pydantic object for structured response handling (more details on this can be found in <FancyInternalLink slug="transcribe-audios-whisper-extract-structured-data-chatgpt"/>).
...
More details on this can be found in <FancyInternalLink slug="transcribe-audios-whisper-extract-structured-data-chatgpt"/>.
...
async def async_call_open_ai_transcription(files, gpt_model=DEFAULT_MODEL):
    ...
    logger.info(f"Processing {len(files)} calls to OpenAI asynchronously {msg_jobs}")
...
async def async_call_open_ai_chat(
    ...
    msg_jobs = f"(n_jobs={u.RATE_LIMITER_SEMAPHORE._value})"
    logger.info(f"Processing {df.shape[0]} calls to OpenAI asynchronously {msg_jobs}")
    tasks = [
        call_openai_chat(
...
* <FancyInternalLink slug="transcribe-audios-whisper-extract-structured-data-chatgpt"/>
