...
## 1. Why Airflow?
People usually need to execute some tasks periodically. One common solution is to use <FancyLink linkText="Cron" url="https://es.wikipedia.org/wiki/Cron_(Unix)" dark="true"/> which is a good solution for simple tasks. But the more tasks you need to schedule the more problems I will have, specially if there are dependencies between one another.
...
If you still have examples running you can reset the database:
...
<Notice type="info">
  We are running **airflow** with `ubuntu` user since by default it gets installed to `~/airflow`. You can change the default path and use another user.
</Notice>
...
To keep the code sync with the origin you could create an airflow task but it will fill the logs with useless information. This time ironically is better to use crontab to fetch the code. You can do it with:
