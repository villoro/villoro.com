...
```python
    ...
    async def wait_for_reload(self, reload_id, poll_every=5, max_time=300):
        import time
        deadline = time.monotonic() + max_time

        while time.monotonic() < deadline:
            status = await self._get(url=f"https://.../api/v1/reloads/{reload_id}")
            s = status.get("status", "").upper()
            if s in ("SUCCEEDED",):
                return status
            if s in ("FAILED", "ABORTED"):
                raise RuntimeError(f"Reload failed: {status}")
            await asyncio.sleep(poll_every)

        raise TimeoutError(f"Reload {reload_id} not finished after {max_time}s")
```
