...
* Distribution of **join types** (INNER, LEFT, CROSS) to identify common patterns or misuse.
* Frequency of **`SELECT *`** to quantify non-selective queries.
...
* Enforce limits on nesting or cross joins.
* Encourage explicit column selection.
* Track usage of complex CTEs and window functions for maintainability.
...
* **Quality scoring:** create a query-level quality index combining static metrics and execution performance.
